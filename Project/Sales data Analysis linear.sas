...
title2 h=10pt " ";
title3 j=Center h=8pt color=Black Bold "This section analyses Product wise total Number of Transaction Per Product in each region by Cross tabulation method and
...
proc sgplot data=Month_Revenue_analysis;
	...
	xaxis label="Month" valueattrs=(size=10 weight=bold) labelattrs=(Color=Black weight=Bold size=11 style=Italic);
...
proc sgplot data=Unit_Sold_analysis;
	...
	yaxis label="Name of the Products" valueattrs=(size=10 weight=bold) 
		labelattrs=(Color=Black weight=Bold size=11 style=Italic);
title "Highest Units of Product sold ";
run;
