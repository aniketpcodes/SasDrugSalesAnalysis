...
/* Transaction analysis per product + cross-tab per region (their PROC FREQ) */
ods noproctitle;
title j=Center "Total Number of Transaction analysis Per Product & Per Region";
proc freq data=Sales_Derivation;
	tables Product_Name /nocum out=Sales_Freq;
run;
...
title j=Center "Total Number of Transaction analysis Per Product in each region";
proc freq data=Sales_Derivation;
