...
/* Top Performing Product by Profit (their PROC SQL) */
...
/* Percentage contribution of Net Revenue per Region (their correlated subquery) */
...
proc sql;
	Create Table Region_Revenue_analysis as select Region, sum(Net_Revenue) as
		...
		Sales_Derivation group by Region order by Calculated Percentage_Contribution desc;
quit;
